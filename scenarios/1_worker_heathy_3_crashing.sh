...
NUM_WORKERS=1
NUM_FAULTY_WORKERS=3

# Docker image names
WORKER_IMAGE="worker:release"
FAULTY_WORKER_IMAGE="worker:crashing"
...
echo "Starting faulty workers..."
for i in $(seq 1 $NUM_FAULTY_WORKERS); do
    container_id=$(docker run -d --rm "$FAULTY_WORKER_IMAGE")
    echo "Faulty worker $i started with container ID: $container_id"
done
